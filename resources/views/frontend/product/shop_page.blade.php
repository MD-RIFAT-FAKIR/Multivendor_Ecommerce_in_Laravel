...
<script type="text/javascript">
    $(document).ready(function() {
        if($('#slider-range').length > 0) {
            ...
            // get existing price range (from hidden input), or fallback
            let price_range = $("#price_range").val();
            let values = [];

            if(price_range !== "") {
                values = price_range.split('-').map(Number); // e.g. [108, 599]
            } else {
                values = [min_price, max_price]; // default full range
            }
